...
const fetchApps = async () => {
  try {
    ...
    if (res.data.length > 0) {
      ...
      appStateStore.setCurrentApp(defaultApp)
    }
  } catch (error) {
...
const handleAppChange = (appId: number) => {
  ...
  if (app) {
    appStateStore.setCurrentApp(app)
...
onMounted(() => {
  if (apps.value.length === 0) {
    ...
    currentAppId.value = appStateStore.currentApp?.id || apps.value[0]?.id
  }
})
</script>
